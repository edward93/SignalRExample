...
using PushAPI.API.ViewModels;
using PushAPI.DAL.Entities;
using PushAPI.Infrastructure.Enums;
using PushAPI.Infrastructure.Helpers;
...
namespace PushAPI.API.Controllers
{
    public class TokenController : BaseController
    {
            ...
            IHostingEnvironment env) : base(logger, settings, env)
        {
            ...
            _options.Audience = "PushAPI.APIAud";
            _options.Issuer = "PushAPI.API";
        }
        ...
        public async Task<IActionResult> Post([FromForm] TokenViewModel model)
        {
            ...
            try
            {
                ...
                var result = await _userService.GetEntityAsync(c => c.Email == model.UserName);
                if (result == null)
                {
